...
def random_gell_mann(GllM, iteracion):
    """
    ...
    GMm = GllM
    N = GMm.shape[2]
    # seleccionamos m  matrices random a medir
...
def compressed_sensing(prob, A, epsilon=0.001):
    ...
    di2 = A.shape[0] #base de medicion
    di = int(np.sqrt(di2))
...
def tomography(dens, GellM, n_bases, potencia, iteraciones,index):
    total_matrix = []

    gm, ind = random_gell_mann(GellM, iteraciones)
    for bases in range(len(n_bases)):
        ...
        for n_exper in potencia:
            ...
            for iter in range(iteraciones):
                nu_exp = exper_dim
                # mediciones
                obs = setmatrix2vec(gm[bases][iter])
                # medimos los valores esperados de los observables
                evals = setexpval(dens, gm[bases][iter], nu_exp = nu_exp)
    ...
    np.save('datos_bases_20_alphas_' + str(index) + '.npy', total_matrix)
    np.save('datos_GMM_20_alphas_' + str(index) + '.npy', GellM)
...
potencia = [13]
# potencia = np.arange(5,26)
n_bases = np.arange(20,21)
# n_bases = np.arange(2,di*di+1)
entrada_canales = np.array([2, 3]) # estado

GellM = gell_mann(di) # Crear matrices de gellman

partition = 50
alphas = np.array([sqrt(float(k/partition)) for k in range(partition+1)])
...
for index in range(alphas.shape[0]):
    ...
    dens = choi_matrix(bms_dim, entrada_canales, entrada_coefs) # Solo bms_dim = 4 permitido por ahora
    total_matr = tomography(dens, GellM, n_bases, potencia, iteraciones, index)
    #total_orden, minimos = comparacion_bases_gm(total_matr, index)

    alphas_matr.append(total_matr)

    #print(len(total_matr)) #alphas y betas distintos
    #print(len(total_matr[0])) #num bases
    #print(len(total_matr[0][0])) #num pot exp
    #print(len(total_matr[0][0][0])) # 0 = fidelidad, 1= matrixes, 2 = gm_matrices
    #print(len(total_matr[0][0][0][0])) #elements of iteracion/montecarlo

    inspect_matr = alphas_matr[index][0][0][1][0]
    ...
    fidelidad_estado = np.dot(entrada_coefs, est_input_state)
    recuperation_matr.append([fidelidad_estado, fidelidad_multiport, entrada_coefs, est_input_state, partial_bs_T])

    print('Se termino la alphas: '+ str(index+1)+' de ' + str(partition))
